...
{	  
	if(mode==0)
	{
		sprintf((char*)pname,"0:PHOTO/PIC%02d%02d%02d%02d%02d%02d.bmp",RTC_DateStruct.RTC_Year,RTC_DateStruct.RTC_Month,RTC_DateStruct.RTC_Date,RTC_TimeStruct.RTC_Hours,RTC_TimeStruct.RTC_Minutes,RTC_TimeStruct.RTC_Seconds);//首选保存在SD卡
	}else 
	{
		sprintf((char*)pname,"0:PHOTO/PIC%02d%02d%02d%02d%02d%02d.jpg",RTC_DateStruct.RTC_Year,RTC_DateStruct.RTC_Month,RTC_DateStruct.RTC_Date,RTC_TimeStruct.RTC_Hours,RTC_TimeStruct.RTC_Minutes,RTC_TimeStruct.RTC_Seconds);		//首选保存在SD卡
	}
} 
//OV2640拍照jpg图片
//返回值:0,成功
...
u8 ov2640_jpg_photo(u8 *pname)
{
	...
	DCMI_DMA_Init((u32)&TFTLCD->LCD_DATA,0,1,DMA_MemoryDataSize_HalfWord,DMA_MemoryInc_Disable);//DCMI DMA配置   
	gui_memin_free(f_jpg); 
	gui_memin_free(jpeg_buf0);
	gui_memin_free(jpeg_buf1);
	gui_memex_free(jpeg_data_buf); 
...
{	
	...
	{ 
		...
		My_DCMI_Init();			//DCMI配置
		DCMI_DMA_Init((u32)&TFTLCD->LCD_DATA,0,1,DMA_MemoryDataSize_HalfWord,DMA_MemoryInc_Disable);//DCMI DMA配置  
		OV2640_OutSize_Set(tftlcd_data.width,tftlcd_data.height);//设置摄像头输出尺寸为LCD的尺寸大小 
		DCMI_Start(); 			//启动传输 
		delay_ms(200);	
...
//				LCD_Display_Dir(0);
				...
				{ 
					...
					{
						case 0://场景设置	  
							res=app_items_sel((tftlcd_data.width-160)/2,(tftlcd_data.height-72-32*5)/2,160,72+32*5,(u8**)camera_l10fun_table[gui_phy.language],5,(u8*)&l10sel,0X90,caption);//单选
							sw_ov2640_mode();//切换为OV2640模式
							if(res==0)
							{
							...
							break;
						case 1://特效设置		 
							res=app_items_sel((tftlcd_data.width-160)/2,(tftlcd_data.height-72-32*7)/2,160,72+32*7,(u8**)camera_l11fun_table[gui_phy.language],7,(u8*)&l11sel,0X90,caption);//单选
							sw_ov2640_mode();//切换为OV2640模式
							if(res==0)
							{
						...
						case 5://对比度设置
							...
							res=app_items_sel((tftlcd_data.width-160)/2,(tftlcd_data.height-72-32*5)/2,160,72+32*5,(u8**)camera_l125fun_table[gui_phy.language],5,(u8*)&selx,0X90,caption);//单选
							sw_ov2640_mode();//切换为OV2640模式
							if(res==0)
							{
...
//				LCD_Display_Dir(1);
			...
			if(key)
			{
				...
				switch(key)
				{
					case KEY_UP_PRESS:	//KEY_UP按下,BMP拍照
						LED1=0;		//DS1亮,提示拍照中
						sw_sdcard_mode();	//切换为SD卡模式
////						if(key==KEY2_PRESS)	//JPEG拍照
//						{
...
//						{
							...
							res=bmp_encode(pname,0,0,tftlcd_data.width,tftlcd_data.height,0);//bmp拍照
//						}	
						if(res)//拍照失败了
						{
							if(res==0XFF)window_msg_box((tftlcd_data.width-200)/2,(tftlcd_data.height-100)/2,200,100,(u8*)camera_failmsg_tbl[1][gui_phy.language],(u8*)APP_REMIND_CAPTION_TBL[gui_phy.language],12,0,0,0);//内存错误
							else if(res==0XFD)window_msg_box((tftlcd_data.width-200)/2,(tftlcd_data.height-100)/2,200,100,(u8*)camera_failmsg_tbl[2][gui_phy.language],(u8*)APP_REMIND_CAPTION_TBL[gui_phy.language],12,0,0,0);//数据错误
							else window_msg_box((tftlcd_data.width-200)/2,(tftlcd_data.height-100)/2,200,100,(u8*)camera_failmsg_tbl[0][gui_phy.language],(u8*)APP_REMIND_CAPTION_TBL[gui_phy.language],12,0,0,0);//提示SD卡是否存在
						}else
						{
						...
						if(scale==0)
						{
							OV2640_ImageWin_Set((1600-tftlcd_data.width)/2,(1200-tftlcd_data.height)/2,tftlcd_data.width,tftlcd_data.height);//1:1真实尺寸
							OV2640_OutSize_Set(tftlcd_data.width,tftlcd_data.height); 
						}else 
						{
						...
						OV2640_OutSize_Set(tftlcd_data.width,tftlcd_data.height); 	
						break;
//					case KEY2_PRESS:	//KEY2按下,缩放/1:1显示(不缩放)
//						scale=!scale;  
//						if(scale==0)
//						{
//							OV2640_ImageWin_Set((1600-tftlcd_data.width)/2,(1200-tftlcd_data.height)/2,tftlcd_data.width,tftlcd_data.height);//1:1真实尺寸
...
//							OV2640_OutSize_Set(tftlcd_data.width,tftlcd_data.height);  
//						} 
//						window_msg_box((tftlcd_data.width-200)/2,(tftlcd_data.height-80)/2,200,80,(u8*)camera_scalemsg_tbl[scale][gui_phy.language],(u8*)APP_REMIND_CAPTION_TBL[gui_phy.language],12,0,0,0);
...
//						break;
					case KEY2_PRESS://JPG图片分辨率设置
						...
						window_msg_box((tftlcd_data.width-200)/2,(tftlcd_data.height-80)/2,200,80,psn,(u8*)APP_REMIND_CAPTION_TBL[gui_phy.language],12,0,0,0);
 						while(KEY_Scan(1)==0&&tcnt<80)//等待800ms,如果没有按键按下的话.
						{
							delay_ms(10);
							tcnt++;
