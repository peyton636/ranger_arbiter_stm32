...
void my_mem_set(void *s,u8 c,u32 count)  
{  
    u8 *xs = s;  
    while(count--)*xs++=c;  
}	   
...
{  
    my_mem_set(mallco_dev.memmap[memx], 0,memtblsize[memx]*2);//内存状态表数据清零  
	my_mem_set(mallco_dev.membase[memx], 0,memsize[memx]);	//内存池所有数据清零  
...
{  
    ...
    {  									   
	    my_mem_cpy((void*)((u32)mallco_dev.membase[memx]+offset),ptr,size);	//拷贝旧内存内容到新内存   
