...
//		printf("%s\r\n",buf);
		sprintf((char *)rtc_buf,"%.2d:%.2d:%.2d",RTC_TimeStruct.RTC_Hours,RTC_TimeStruct.RTC_Minutes,RTC_TimeStruct.RTC_Seconds);
		if(rtc_showflag==1)
		{
			FRONT_COLOR=UI_FRONTCOLOR;
			BACK_COLOR=UI_BACKCOLOR;
			LCD_ShowString(tftlcd_data.width-(10+8*TOP_STATUS_NAME_FONT_SIZE/2),2,tftlcd_data.width,TOP_STATUS_NAME_HEIGHT,TOP_STATUS_NAME_FONT_SIZE,rtc_buf);
		}	
