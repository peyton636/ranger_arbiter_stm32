...
//							else audiodev.status|=0X01;
						...
						else if(key==KEY1_PRESS)//下一首
						{
							res=1;
							break;
						}
						else if(key==KEY2_PRESS)//音量+
						{
							if(audio_mvol<64)
								audio_mvol++;
							else audio_mvol=0;
						...
						audio_msg_show(10,gui_phy.tbheight+30,wavctrl.totsec,wavctrl.cursec,wavctrl.bitrate);
						t++;
						if(t==20)
						{
							t=0;
