...
{	 
	...
	if(oldsec!=cursec)	//需要更新显示时间
	{
		...
		sprintf((char*)buf,"播放时间:%02d:%02d:%02d/%02d:%02d:%02d",cursec/3600,(cursec%3600)/60,cursec%60,totsec/3600,(totsec%3600)/60,totsec%60);
 		LCD_ShowFontString(10,90,tftlcd_data.width-10,16,buf,16,0);	//显示歌曲名字
		myfree(SRAMIN,buf);		
	} 		 
}
//显示当前视频文件的相关信息 
...
{	  
	...
	sprintf((char*)buf,"声道数:%d,采样率:%d",aviinfo->Channels,aviinfo->SampleRate*10); 
 	LCD_ShowFontString(10,50,tftlcd_data.width-10,16,buf,16,0);	//显示歌曲名字
	sprintf((char*)buf,"帧率:%d帧",1000/(aviinfo->SecPerFrame/1000)); 
 	LCD_ShowFontString(10,70,tftlcd_data.width-10,16,buf,16,0);	//显示歌曲名字
	myfree(SRAMIN,buf);	  
}
//视频基本信息显示
//name:视频名字
...
{		
	...
	LCD_ShowFontString(10,10,tftlcd_data.width-10,16,buf,16,0);//显示文件名
	sprintf((char*)buf,"索引:%d/%d",index,total);	
	LCD_ShowFontString(10,30,tftlcd_data.width-10,16,buf,16,0);//显示索引 		  	  
	myfree(SRAMIN,buf);		
}
...
void video_play(void)
{
	...
	u32 *voffsettbl;	//视频文件off block索引表
 	...
 	{	    
		LCD_ShowFontString(60,190,240,16,"VIDEO文件夹错误!",16,0);
		delay_ms(200);				  
	...
	totavinum=video_get_tnum("0:/VIDEO"); //得到总有效文件数
 	...
 	{	    
		LCD_ShowFontString(60,190,240,16,"没有视频文件!",16,0);
		delay_ms(200);				  
	...
	vfileinfo=(FILINFO*)mymalloc(SRAMIN,sizeof(FILINFO));//为长文件缓存区分配内存
 	...
 	{	    
		LCD_ShowFontString(60,190,240,16,"内存分配失败!",16,0);
		delay_ms(200);				  
		LCD_Fill(60,190,240,146,WHITE);//清除显示	     
		delay_ms(200);				  
	}  	 
...
{   
	...
	{ 
		...
		if(res==0)
		{
			...
			TIM6_Init(avix.SecPerFrame/100-1,8400-1);//10Khz计数频率,加1是100us 
			...
			res=mjpegdec_init((tftlcd_data.width-avix.Width)/2,110+(tftlcd_data.height-110-avix.Height)/2);//JPG解码初始化 
			...
			{					
				...
				key=KEY_Scan(1);
				if(key==KEY2_PRESS)//KEY0/KEY2按下,播放下一个/上一个视频
				{
					res=1;
					break; 
				}else if(key==KEY_UP_PRESS)//快进/后退
				{
					I2S_Play_Stop();//关闭音频
			...
			I2S_Play_Stop();	//关闭音频
			TIM6->CR1&=~(1<<0); //关闭定时器6
			LCD_Set_Window(0,0,tftlcd_data.width,tftlcd_data.height);//恢复窗口
			mjpegdec_free();	//释放内存
			f_close(favi); 
		}
	}
...
//		else if(key==KEY1_PRESS)//快退
//		{
//			if(fpos>delta)fpos-=delta;
//			else fpos=0; 
//		}
		else if(key==KEY1_PRESS)//返回
			break;
		f_lseek(favi,fpos);
