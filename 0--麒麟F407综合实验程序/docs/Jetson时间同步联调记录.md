...
| **文档版本** | v1.4.0                                                                                  |
| **最后更新** | **2026-06-18**                                                                          |
| **链路**   | **以太网 UDP（主）** + RS232 / USART2（备）                                                  |
| **协议**   | [Jetson_CAN协议.md §8.1](./Jetson_CAN协议.md) / [Jetson_BLOB协议_v2.md](./Jetson_BLOB协议_v2.md) |
| **联调结论** | **单调时钟对时 + Jetson 侧虚拟时间轴已验收**（RS232 2026-06-15；**以太网 2026-06-18**）；MCU 四路 stamp **经 BLOB 已实现**（见 §5）；**收尾状态见 §4、验收见 §7** |
...
| 项 | 值 |
|----|-----|
| **MCU IP** | 192.168.10.30（静态） |
| **Jetson IP** | 192.168.10.201 |
| **MCU 收** | UDP **50001**（Jetson 下行 BLOB + 0xA5 服务帧） |
| **MCU 发** | UDP **50002**（MCU 上行 BLOB + 0xA5 服务帧） |
| **时间同步** | 与 RS232 **相同**：`0xA5` 封装 **0x107 请求 / 0x108 响应**，不经 BLOB 0xAB 头 |
| **业务数据** | BLOB v2（0x02 运动 / 0x03 状态 / 0x04 传感器等），各帧 **byte0~3 = timestamp_ms** |
...
- `APP/jetson_eth/jetson_eth.c` — UDP 收包 → 0xA5 服务帧入队 / BLOB 解析
- `APP/jetson_can/jetson_can.c` — `HandleTimeRequest()`（与 RS232 **共用**）
- `APP/freertos/rtos_tasks.c` — `vJetsonTask` 锁外处理服务队列后回 0x108

调试串口仍用 **USART1**（PA9/PA10 @ 115200）；以太网 TimeSync 成功时可见 `[ETH SVC] first 0x107 cmd=0x02` 等 log。

> **USART1 有 log ≠ Jetson 链路正常**；以太网验收以 **`/jetson_eth/time_sync`** 为准。
...
### 2.4.1 2026-06-18 以太网联调通过（已达成）

| 项目 | 结果 |
|------|------|
| `jetson_eth_bringup` + UDP 50001/50002 | BLOB 上下行 + 0xA5 服务帧 |
| `/jetson_eth/time_sync` | `offset_valid=true`，RTT 10~40 ms |
| `/jetson_eth/sonar_stamped` | ~80 Hz，`header.stamp` 与 odom 同轴 |
| safety_state | NORMAL |
| 四路独立 `mcu_stamp_ms` | **代码已实现**；需新固件实车复验 |
...
`MCU_PROC_100US = (tick_tx - tick_rx) × 10`，上限 255（25.5 ms）。`TIME_FLAGS`：bit0 = GPS UTC 有效；**bit1 = RTC 有效**（`RTC_BKP_DR0==0x5050`）；bit2 `RTC_SYNCED_FROM_GPS` 未实现。
...
### 3.11 以太网与 RS232 共用时间同步逻辑

```text
Jetson eth_gateway                    MCU
  │ UDP :50001  A5 01 07 [8B]  ──────>│ JetsonEth_HandleSvcRx → 服务队列
  │                                    │ vJetsonTask 锁外
  │                                    │   JetsonCAN_HandleTimeRequest
  │                                    │   JetsonLink_Deliver8(0x108)
  │ UDP :50002  A5 01 08 [8B]  <──────│ JetsonEth_UdpSendRaw
```

| 对比项 | RS232 | 以太网 |
|--------|-------|--------|
| 物理层 | USART2 115200 | UDP 50001/50002 |
| 0x107/0x108 载荷 | 相同 8B | 相同 8B |
| 业务上行 | V3 0x02 或 BLOB | **BLOB v2**（含 stamp 字段） |
| Jetson topic | `/jetson_rs232/time_sync` | **`/jetson_eth/time_sync`** |
| PING RTT 典型值 | 5~30 ms | **10~40 ms**（含 vJetsonTask 20ms 周期） |

**设计原则（收尾定稿）**：

| 侧 | 职责 |
|----|------|
| **MCU** | 事件时刻用本机 `SYSTEM_TICK_MS` 打戳；收到 PING **立刻**（下一 JetsonTask 周期）回 `mcu_tick_rx`；**不改 RTC / 不调 tick** |
| **Jetson** | PING 算 `offset_ms`；各模块 `header.stamp` = 虚拟 MCU 时刻（`mono + offset`）；**不回传 offset 给 MCU** |
| **MCU 重启** | tick 归零 → Jetson offset 大幅跳变 → Jetson 侧重置滤波；MCU 无额外动作 |

---

## 4. 工作项状态（2026-06-18 收尾）
...
| 项 | RS232 | 以太网 | 说明 |
|----|:-----:|:------:|------|
| 0x107/0x108 START/PING/QUERY/STOP | ✅ | ✅ | 协议与代码共用 `jetson_can.c` |
| gateway + `time_sync` topic | ✅ | ✅ | `offset_valid=true` 实车通过 |
| 1 Hz PING + 10 s QUERY | ✅ | ✅ | |
| BLOB `header.stamp` 用 offset 换算 | — | ✅ | Jetson P1 |
| eth_gateway BLOB 上下行 | — | ✅ | Jetson P2 |
| GPS `/fix` stamp 用 offset | — | ✅ | Jetson P3；室内 `utc=0` 正常 |
| `/jetson_eth/sonar_stamped` | — | ✅ | Jetson P4 |
| MCU BLOB 四路 `sonar_stamp_ms` | — | ✅ | `distance_sensor.c` + `agv_blob_pack.c` |
| MCU 发帧瞬间 `timestamp_ms` | — | ✅ | `BlobPack_SendLink()` 在 `BlobLink_Send` 前写 byte0~3 |
| `TIME_FLAGS` bit1 RTC_VALID | ✅ | ✅ | `JetsonCAN_BuildTimeFlags()` |
...
| 项 | 说明 |
|----|------|
| **1 h soak** | `jetson_time_soak.py --link-type eth --duration 3600`；工具就绪，待长跑留档 |
| **Phase2 协议级 TX_TICK_MS** | V3/业务帧专用字段；当前 BLOB byte0~3 已覆盖发帧时刻 |
| **GPS → RTC 硬校时** | 可选；`RTC_SYNCED_FROM_GPS` 未实现 |
| **0x107 子命令回传 offset** | 协议未定义；MCU **不应**用 offset 改 tick |
| **Linux settimeofday** | 不做；传感器对齐看 ROS `header.stamp`，不是 `date` |
| **V3 type=0x04 专帧** | RS232 纯 V3 路径备选；**以太网 BLOB 0x03/0x04 已承载 stamp**，见 §5 |
| **户外 GPS fix 复验 QUERY** | 有 fix 时 `utc_unix` 应非 0 |

### 4.3 实车复验建议（收尾）

1. 确认 MCU 已烧录含 **§5 实现** 的固件。
2. 挡一路测距，看 `/jetson_eth/sonar_stamped` 四路 **`mcu_stamp_ms` 是否独立变化**（旧固件可能四路相同）。
3. 多 topic `header.stamp` 互差应在 **毫秒～十几毫秒**，不能差秒级。

---

## 5. 四路测距时间戳（需求、实现与 V3 备选）

> **与时间同步的关系**：**§3** 的 PING/offset 解决「Jetson 钟 ↔ MCU 钟」换算；本节解决「四路 sonar **各自何时更新**」——两者叠加后，Jetson 才能把每路距离放到统一时间轴上。

> **2026-06-18 定稿**：**以太网主链路采用 BLOB v2** 携带四路 stamp（非 V3 type=0x04）。RS232 纯 V3 路径仍可按 §5.5 增 0x04 专帧（备选，**当前固件未实现 V3 0x04**）。

### 5.0 已实现（BLOB 路径，2026-06-18）

| 项 | 实现 |
|----|------|
| **打戳时机** | 某路滤波 `stable_mm` **相对上次变化**时，`stable_stamp_ms = xTaskGetTickCount()` |
| **API** | `DistanceSensor_GetStableStampMs(idx)`（`distance_sensor.c/h`） |
| **上送 MSG 0x04** | byte12~27：四路 `stamp_f/b/l/r`（各 u32 BE） |
| **上送 MSG 0x03** | byte22~37：四路 stamp（与距离同帧） |
| **帧时刻** | `BlobPack_SendLink()` 在 `BlobLink_Send` **前一刻**写 payload byte0~3 |
| **协议定义** | [Jetson_BLOB协议_v2.md §3.1 / §4.1](./Jetson_BLOB协议_v2.md) |
| **Jetson 消费** | `eth_gateway` → `/jetson_eth/sonar_stamped`（`mcu_stamp_ms[4]` + offset 换算） |

```text
DistanceSensor_FilterOnNewFrame()
  -> stable_mm 变化 -> stable_stamp_ms = NowMs()
JetsonTask -> BlobPack_SendSensor / SendMcuStatus
  -> PutSonarStamps() + SendLink(timestamp_ms)
eth_gateway -> sonar_stamped.header.stamp
```

**验证**：单路挡板时，对应 `mcu_stamp_ms[i]` 应独立更新；四路全相同多为 **旧固件**。
...
### 5.2 MCU 侧数据结构（BLOB 路径已实现；SonarSnapshot_t 为早期规划）

当前实现 **未** 引入独立 `SonarSnapshot_t`，而是在滤波通道内维护 stamp，经 BLOB 组包上送：

```c
/* distance_sensor.c — 每路滤波通道 */
u32 stable_stamp_ms;   /* stable_mm 最后变化时刻 */

/* agv_blob_pack.c */
BlobPack_PutSonarStamps(p, base_idx);  /* 0x04: base=12; 0x03: base=22 */
BlobPack_SendLink(...);                /* byte0~3 = 发帧瞬间 tick */
```

早期规划（RS232 V3 路径可选）仍保留如下结构供参考：
...
~~替换 `DistSnapshot_Write(f,b,l,r,n)` 仅传 u16~~ — BLOB 路径仍用 `DistSnapshot` 传距离，stamp 由 `DistanceSensor_GetStableStampMs` 按需读取。
...
### 5.5 备选：V3 type=0x04（测距 stamp 专帧，RS232 路径；**以太网未采用**）
...
- [x] 滤波 `stable_mm` 变化处写 `stable_stamp_ms`（**每路独立**）
- [x] BLOB 0x03/0x04 上送 4×u32 stamp（`BlobPack_PutSonarStamps`）
- [x] `BlobPack_SendLink` 发帧前写 `timestamp_ms`
- [x] **仅在 JetsonTask 发 TX**；stamp 路径不进 ISR
- [ ] 实车：单路挡板验证四路 stamp 独立（需新固件）

**MCU（V3 0x04 备选，未做）**

- [ ] `USART3_SendV3SonarStampFrame()` 或 0x111/0x112 双服务帧

**Jetson / gateway**

- [x] 解析 BLOB 0x03/0x04 stamp 字段
- [x] 发布 `/jetson_eth/sonar_stamped` + `/jetson_eth/time_sync`
- [x] `header.stamp` 用 `offset_ms` 换算虚拟 MCU 时间轴
- [ ] 1 h soak 留档

**验证**

- [ ] 静止/挡板：四路 stamp 可不同
- [ ] PING 1 Hz 下 `offset_valid` 稳定；BLOB 上行不中断（§2.5 回归）
- [ ] 多 topic `header.stamp` 互差 ~ms 量级（见 **§7**）
...
**禁止**：为降低延迟在 USART2 RX ISR 内发送 stamp 帧（会复现 §2.5 PING 灾难）。

---

## 7. 验收清单（Jetson / MCU 分工）

### 7.1 Jetson 侧

**前提**：`jetson_eth_bringup.launch.py` 已起，等待 10~20 s TimeSync 收敛。

**① 对表是否成功（必看）**

```bash
ros2 topic echo /jetson_eth/time_sync
```

| 字段 | 期望 |
|------|------|
| `offset_valid` | **true** |
| `sample_accepted` | 多为 true |
| `rtt_ms` | 一般 10–30 ms，偶发 40 ms 可接受 |
| `offset_ms` | 大负数正常（两机开机时刻不同） |

若长期 `offset_valid=false`，后续所有虚拟时间不可信。

**② 多 Topic 是否同轴**

```bash
ros2 topic echo /jetson_eth/v3_status --field header.stamp
ros2 topic echo /jetson_eth/sonar_stamped --field header.stamp
ros2 topic echo /odom --field header.stamp
```

同一物理时刻的数据，`header.stamp` 互差应在 **毫秒～十几毫秒**，不能差秒级。

**③ 不要用 `date` 验传感器** — 墙钟未改；只看 ROS `header.stamp`。

**④ 可选 soak**

```bash
python3 tools/jetson_time_soak.py --link-type eth --duration 600
```
...
|------|------|
| 时间基准 | 继续用本机 tick，**不改钟** |
| 数据上报 | BLOB 带 `timestamp_ms` + 四路 `sonar_stamp_ms` |
| TimeSync | 收到 PING 回 0x108（`mcu_tick_rx`） |
| 收 Jetson offset | **不需要**；协议未定义回传 |
| 重启后 | 正常 PING 即可；Jetson 侧处理 offset 跳变 |

### 7.3 谁负责什么

| 侧 | 职责 |
|----|------|
| **MCU** | 事件时刻 → 本机 tick 打戳；PING/QUERY 供 Jetson 算 offset |
| **Jetson** | PING → offset → 各模块 `header.stamp`；**不回传 MCU** |
| **验收** | `offset_valid=true` + 多 topic stamp 互差 &lt; ~20 ms |
...
- [Jetson_BLOB协议_v2.md](./Jetson_BLOB协议_v2.md)
- [硬件连接与通信协议.md](./硬件连接与通信协议.md)
- [以太网与WiFi接入方案.md](./以太网与WiFi接入方案.md)
...
| **v1.4.0** | **2026-06-18** | **收尾定稿**：§1.1 以太网链路；§3.11 ETH/RS232 共用逻辑与分工原则；§4 状态表更新（BLOB stamp 已实现）；§5.0 已实现路径；§7 验收清单；TIME_FLAGS bit1 |
