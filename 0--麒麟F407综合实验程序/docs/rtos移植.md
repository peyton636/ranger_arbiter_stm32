...
| **GpsTask** | **2** | 100ms | 512w | **仅** GPS：`GPS_Process()` + `GPS_PrintStatus()` |
...
GpsTask ──► GPS_Process + GPS_PrintStatus（串口）；LCD 未接 GPS 显示
```
...
- [ ] Jetson 下行 / 上行（JetsonTask）
- [ ] 测距 → 快照 → 运动 / 蜂鸣（SensorTask）；日志/Print 在 UiTask
- [ ] CAN 反馈 → 仲裁 → LCD 底盘区（CanTask + UiTask）
- [ ] 按键切蜂鸣（KeyTask + UiTask）
- [ ] GPS 打印（GpsTask）
...
```
├─ 串口中断：帧完整后整帧入队（24 字节），Task 侧不再轮询 GetJetsonFrame 缓冲区
├─ CanTask / JetsonTask：阻塞收队列，再 Parse / Process
├─ arb_state 互斥量（阶段 1 已建）沿用；g_*_dist_mm 可继续临界区或改只读快照结构体
├─ NVIC 优先级：FromISR API 的中断优先级 ≥ configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY(5)
└─ 验证：高负载下串口不丢帧 + 数据不乱
```
